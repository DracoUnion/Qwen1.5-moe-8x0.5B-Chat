...
# MOE 的架构
class Qwen2SparseMoe(nn.Module):
    """
    ...
    def load_state_dict(self, state_dict, strict=True):
        # 覆盖 load_state_dict 兼容原有参数
        for pref in ['gate_proj', 'up_proj', 'down_proj']:
            pref_param_names = [name for name in state_dict if name.startswith(pref + '.')]
            for n in pref_param_names:
                for i in range(self.num_experts):
                    state_dict[f'experts.{i}.{n}'] = state_dict[n]
                del state_dict[n]
        
        if not any(name for name in state_dict if name.startswith('gate.')):
            for n, p in self.gate.state_dict().items():
                state_dict[f'gate.{n}'] = p
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.mlp = Qwen2SparseMoe(config)
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def chat(self, tok, ques, history=[], **kw):
        iids = tok.apply_chat_template(
            history + [{'role': 'user', 'content': ques}], 
            add_generation_prompt=1,
        )
        oids = self.generate(
            inputs=torch.tensor([iids]).to(self.device),
            **(self.generation_config.to_dict() | kw),
        )
